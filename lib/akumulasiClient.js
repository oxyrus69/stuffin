...
export function fillAkumulasi(xlsxBuf, sewParsed, assParsed, workHours = null, mode = 'regular', lineHours = null) {
  ...
  try {
    ...
    if (!wbXmlRaw.includes(`name="${sheetName}"`)) {
      ...
      // buat sheet XML — pakai style yang sama dengan sheets mingguan (reuse xf indices)
      // s=3  → header kuning (font bold12 + fill kuning + border thin, centered wrap) — Row4 di weekly
      // s=10 → header alternatif (fill abu) — Row6 di weekly
      // s=12 → Line code (bold, border thin, centered) — A7 di weekly
      // s=13 → data angka (font normal 12, border thin, centered)
      // s=14 → data angka/ % (sama + numberFormat)
      // s=16/18 → section/grand total (fill berwarna)
      const S_TITLE = 3, S_HEADER = 3, S_SECTION = 9, S_LINE = 12, S_DATA = 13, S_PCT = 14, S_TOTAL = 18;
      const inlineCell = (ref, text, s) => `<c r="${ref}" t="inlineStr"${s!=null?` s="${s}"`:''}><is><t>${xmlEscapeInline(text)}</t></is></c>`;
      const numCell = (ref, val, s) => `<c r="${ref}"${s!=null?` s="${s}"`:''}><v>${val}</v></c>`;
      let rowsXml = '';
      const addRow = (r, cellsXml, ht) => { rowsXml += `<row r="${r}"${ht?` ht="${ht}" customHeight="1"`:''} x14ac:dyDescent="0.25">${cellsXml}</row>`; };
      // Judul merge A1:E1 — kuning tebal seperti weekly Row4, tinggi 30
      addRow(1, inlineCell('A1', `OUTPUT SEWING, INPUT ASB VS TARGET PRODUKSI — ${periodLabel}`, S_TITLE) + inlineCell('B1','',S_TITLE) + inlineCell('C1','',S_TITLE) + inlineCell('D1','',S_TITLE) + inlineCell('E1','',S_TITLE), 30);
      // baris kosong spacer biar mirip jarak weekly (opsional, 5pt)
      // header kolom — kuning, bold, border thin, tinggi 30, wrap
      addRow(2, inlineCell('A2','Line', S_HEADER) + inlineCell('B2','Total Output', S_HEADER) + inlineCell('C2','Total Target', S_HEADER) + inlineCell('D2','Selisih Target & Actual Output', S_HEADER) + inlineCell('E2','%', S_HEADER), 30);
      let rNum = 3;
      // kelompok SEWING header — abu (fill3) seperti sub-header weekly
      if (sewCodes.length) {
        addRow(rNum, inlineCell('A'+rNum,'— SEWING —', S_SECTION) + inlineCell('B'+rNum,'',S_SECTION) + inlineCell('C'+rNum,'',S_SECTION) + inlineCell('D'+rNum,'',S_SECTION) + inlineCell('E'+rNum,'',S_SECTION), 18);
        rNum++;
        for (const row of monthlyRows.filter(x=>x.kind==='sew')) {
          addRow(rNum,
            inlineCell('A'+rNum, row.code, S_LINE) +
            numCell('B'+rNum, row.totalOutput, S_DATA) +
            numCell('C'+rNum, row.totalTarget, S_DATA) +
            numCell('D'+rNum, row.selisih, S_DATA) +
            inlineCell('E'+rNum, row.pct + '%', S_PCT)
          , 15);
          rNum++;
        }
      }
      if (assCodes.length) {
        addRow(rNum, inlineCell('A'+rNum,'— ASSEMBLING —', S_SECTION) + inlineCell('B'+rNum,'',S_SECTION) + inlineCell('C'+rNum,'',S_SECTION) + inlineCell('D'+rNum,'',S_SECTION) + inlineCell('E'+rNum,'',S_SECTION), 18);
        rNum++;
        for (const row of monthlyRows.filter(x=>x.kind==='ass')) {
          addRow(rNum,
            inlineCell('A'+rNum, row.code, S_LINE) +
            numCell('B'+rNum, row.totalOutput, S_DATA) +
            numCell('C'+rNum, row.totalTarget, S_DATA) +
      ...
      // grand total — fill berwarna + bold (S_TOTAL = font4 fill4 border2)
      const grandOut = monthlyRows.reduce((s,x)=>s+x.totalOutput,0);
      const grandTarget = monthlyRows.reduce((s,x)=>s+x.totalTarget,0);
      const grandSelisih = grandOut - grandTarget;
      const grandPct = grandTarget ? Math.round(grandOut/grandTarget*100) : 0;
      rNum++;
      addRow(rNum, inlineCell('A'+rNum,'GRAND TOTAL', S_TOTAL) + numCell('B'+rNum, grandOut, S_TOTAL) + numCell('C'+rNum, grandTarget, S_TOTAL) + numCell('D'+rNum, grandSelisih, S_TOTAL) + inlineCell('E'+rNum, grandPct+'%', S_TOTAL), 18);
      const sheetXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac xr xr2 xr3" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac" xmlns:xr="http://schemas.microsoft.com/office/spreadsheetml/2014/revision" xmlns:xr2="http://schemas.microsoft.com/office/spreadsheetml/2015/revision2" xmlns:xr3="http://schemas.microsoft.com/office/spreadsheetml/2016/revision3" xr:uid="{11111111-1111-1111-1111-111111111113}"><dimension ref="A1:E${rNum}"/><sheetViews><sheetView tabSelected="1" workbookViewId="0" zoomScale="100" zoomScaleNormal="100"><selection activeCell="A1" sqref="A1"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="15" x14ac:dyDescent="0.25"/><cols><col min="1" max="1" width="12" customWidth="1"/><col min="2" max="4" width="14" customWidth="1"/><col min="5" max="5" width="9" customWidth="1"/></cols><sheetData>${rowsXml}</sheetData><mergeCells count="1"><mergeCell ref="A1:E1"/></mergeCells><printOptions horizontalCentered="1"/><pageSetup paperSize="9" orientation="landscape" fitToWidth="1" fitToHeight="1"/><pageMargins left="0.4" right="0.4" top="0.4" bottom="0.4" header="0.2" footer="0.2"/></worksheet>`;
