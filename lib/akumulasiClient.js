...
export function parseDailyFile(workbook) {
  ...
  for (const sn of workbook.SheetNames) {
    ...
    const header = aoa[0] || [];
    const dCols = []; // Dn -> column index
    for (let c = 0; c < header.length; c++) {
      const m = /^D(\d{1,2})$/i.exec(String(header[c] ?? '').trim());
      if (m) dCols.push({ day: parseInt(m[1], 10), col: c });
    }
    if (!dCols.length || !aoa.some((r) => String(r[0] ?? '').trim() === 'Tota')) {
      // not the expected shape; try next sheet
      continue;
    }
