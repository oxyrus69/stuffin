...
export function fillAkumulasi(xlsxBuf, sewParsed, assParsed, workHours = null, mode = 'regular', lineHours = null) {
  ...
  try {
    ...
    if (!wbXmlRaw.includes(`name="${sheetName}"`)) {
      ...
      // Judul merge A1:E1 — kuning tebal seperti weekly Row4, tinggi 30 — pakai '-' bukan em dash agar aman XML
      ...
      const sheetXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><dimension ref="A1:E${rNum}"/><sheetViews><sheetView workbookViewId="0"/></sheetViews><sheetFormatPr defaultRowHeight="15"/><cols><col min="1" max="1" width="12" customWidth="1"/><col min="2" max="4" width="14" customWidth="1"/><col min="5" max="5" width="9" customWidth="1"/></cols><sheetData>${rowsXml}</sheetData><mergeCells count="1"><mergeCell ref="A1:E1"/></mergeCells></worksheet>`;
