...
export function fillAkumulasi(xlsxBuf, sewParsed, assParsed, workHours = null, mode = 'regular', lineHours = null) {
  ...
  try {
    ...
    if (!wbXmlRaw.includes(`name="${sheetName}"`)) {
      ...
      // buat sheet XML
      const headerFill = 'FFFFFF99'; // kuning seperti gambar (approx)
      // helper buat cell
      const inlineCell = (ref, text, style) => `<c r="${ref}" t="inlineStr"${style?` s="${style}"`:''}><is><t>${xmlEscapeInline(text)}</t></is></c>`;
      const numCell = (ref, val, style) => `<c r="${ref}"${style?` s="${style}"`:''}><v>${val}</v></c>`;
      // kita pakai style minimal (0 = normal), header bold via inline? cukup bold via font? keep simple
      let rowsXml = '';
      const addRow = (r, cellsXml, ht) => { rowsXml += `<row r="${r}"${ht?` ht="${ht}" customHeight="1"`:''}>${cellsXml}</row>`; };
      // Judul merge A1:E1
      addRow(1, inlineCell('A1', `OUTPUT SEWING, INPUT ASB VS TARGET PRODUKSI — ${periodLabel}`) + inlineCell('B1','') + inlineCell('C1','') + inlineCell('D1','') + inlineCell('E1',''), 22);
      // header baris 2
      addRow(2, inlineCell('A2','Line') + inlineCell('B2','Total Output') + inlineCell('C2','Total Target') + inlineCell('D2','Selisih Target & Actual Output') + inlineCell('E2','%'));
      // sep baris kosong
      let rNum = 3;
      // kelompok SEWING header
      if (sewCodes.length) {
        addRow(rNum, inlineCell('A'+rNum,'— SEWING —') + inlineCell('B'+rNum,'') + inlineCell('C'+rNum,'') + inlineCell('D'+rNum,'') + inlineCell('E'+rNum,''));
        rNum++;
        for (const row of monthlyRows.filter(x=>x.kind==='sew')) {
          addRow(rNum,
            inlineCell('A'+rNum, row.code) +
            numCell('B'+rNum, row.totalOutput) +
            numCell('C'+rNum, row.totalTarget) +
            numCell('D'+rNum, row.selisih) +
            inlineCell('E'+rNum, row.pct + '%')
          );
          rNum++;
        }
      }
      if (assCodes.length) {
        addRow(rNum, inlineCell('A'+rNum,'— ASSEMBLING —') + inlineCell('B'+rNum,'') + inlineCell('C'+rNum,'') + inlineCell('D'+rNum,'') + inlineCell('E'+rNum,''));
        rNum++;
      ...
      // extra codes jika ada
      const extras = monthlyRows.filter(x=> !['sew','ass'].includes(x.kind) || (extraCodes.includes(x.code)));
      // sudah masuk
      // grand total
