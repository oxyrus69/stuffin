 ...
 */
export function fillAkumulasi(xlsxBuf, sewParsed, assParsed, workHours = null) {
    ...
    for (let i = 0; i < colHeaders.length - 1; i++) {
      if (colHeaders[i].text === 'output' && colHeaders[i + 1].text === 'target') {
        targetCols.push({
          ...
          selisihCol: colNumToLetter(colHeaders[i + 1].colNum + 1),
          percentCol: colNumToLetter(colHeaders[i + 1].colNum + 2)
        });
      }
    }
  ...
  for (const w of weekRows) {
    ...
    let totalSelisihCol = null;
    let totalPercentCol = null;
    if (hdrInner) {
      const cells = [...hdrInner.matchAll(/<c r="([A-Z]+)\d+"[^>]*t="s"[^>]*><v>(\d+)<\/v>/g)];
      for (const m of cells) {
        const col = m[1];
        ...
        else if (txt.includes('selisih target')) totalSelisihCol = col;
      }
      // Total % is typically right after Total Selisih
      if (totalSelisihCol) {
        totalPercentCol = colNumToLetter(colLetterToNum(totalSelisihCol) + 1);
      }
    }
    ...
    for (let r = w + 3; r <= w + 49; r++) {
      ...
      if (source) {
        ...
        if (rec) {
          ...
          // Fill Target, Selisih, and % for each day if workHours provided
          if (workHours && targetColMap.length > 0) {
            let totalTarget = 0;
            let totalOutput = rowTotal;
            for (let di = 0; di < targetColMap.length && di < dayCols.length; di++) {
              const tc = targetColMap[di];
              if (!tc) continue;
              const hours = workHours[di] || 8;
              const target = hours * TARGET_PER_HOUR;
              totalTarget += target;
              // Set Target cell
              newRowXml = setCellInRow(newRowXml, tc.targetCol + r, target);
              filledCells++;
              // Use tracked daily output
              const outputVal = dailyOutputs[di] || 0;
              // Calculate Selisih = Output - Target
              const selisih = outputVal - target;
              newRowXml = setCellInRow(newRowXml, tc.selisihCol + r, selisih);
              filledCells++;
              // Calculate % = (Output / Target) * 100
              const percent = target > 0 ? Math.round((outputVal / target) * 100) : 0;
              newRowXml = setCellInRow(newRowXml, tc.percentCol + r, percent + '%');
              filledCells++;
            }
            // Set Total Target
            if (totalTargetCol) {
            ...
            // Set Total Selisih
            if (totalSelisihCol) {
              const totalSelisih = totalOutput - totalTarget;
              newRowXml = setCellInRow(newRowXml, totalSelisihCol + r, totalSelisih);
              filledCells++;
            }
            // Set Total %
            if (totalPercentCol && totalTarget > 0) {
              const totalPercent = Math.round((totalOutput / totalTarget) * 100);
              newRowXml = setCellInRow(newRowXml, totalPercentCol + r, totalPercent + '%');
              filledCells++;
            }
          }
        }
        ...
        if (sums) {
          ...
          // Fill Target for SEW/ASS section sums
          if (workHours && targetColMap.length > 0) {
            let totalTarget = 0;
            for (let di = 0; di < targetColMap.length && di < dayCols.length; di++) {
              const tc = targetColMap[di];
            ...
            if (totalSelisihCol) {
              const totalSelisih = total - totalTarget;
              newRowXml = setCellInRow(newRowXml, totalSelisihCol + r, totalSelisih);
              filledCells++;
            }
            if (totalPercentCol && totalTarget > 0) {
              const totalPercent = Math.round((total / totalTarget) * 100);
              newRowXml = setCellInRow(newRowXml, totalPercentCol + r, totalPercent + '%');
              filledCells++;
            }
          }
