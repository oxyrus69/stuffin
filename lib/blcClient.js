...
function sortRowsFiscal(rows, mergedHeader) {
...
export async function processBlc({ jitFiles = [], stuffingFile = null, onProgress = () => {} }) {
  ...
  sortRowsFiscal(rows, mergedHeader);

  const fallbackPeriod = `${new Date().getFullYear()} ${new Date().getMonth() + 1}`;
  ...
  return emitOutput({
    ...
    warnings,
    onProgress,
    jitCount: jitFiles.length,
  });
}

/* ─── STEP 4: assemble output (shared by default & review paths) ───
...
function emitOutput({ mergedHeader, rows, period, stuffingFile, warnings, onProgress, jitCount }) {
  const now = new Date();
...
/* ─── review flow ───
   stageJitFiles: parse semua file JIT dan kembalikan SEMUA baris beserta
   status proposalnya (masuk/dibuang + alasan) tanpa mengunci keputusan.
   assembleFromSelection merakit output hanya dari baris yang dipilih user. */

export async function stageJitFiles({ jitFiles = [], onProgress = () => {} }) {
  const warnings = [];
  ...
  onProgress('Membaca file JIT...');
  const files = [];
  const firstSeen = new Map(); // ordNo -> { name, fileIdx }

  for (let fileIdx = 0; fileIdx < jitFiles.length; fileIdx++) {
    const file = jitFiles[fileIdx];
    let wb;
    let textPeriod = null;
    try {
      const parsed = parseWorkbookBuffer(file.bytes);
    ...
    const ordCol = extracted.header.findIndex((h) => norm(h) === 'ordno');
    const styleCol = extracted.header.findIndex((h) => norm(h) === 'styleno');
    const rows = extracted.rows
      .map((raw, i) => {
        const ordNo = ordCol >= 0 ? String(raw[ordCol] ?? '').trim() : '';
        return {
          i,
          ordNo,
          styleNo: styleCol >= 0 ? String(raw[styleCol] ?? '').trim() : '',
          isNb: !!ordNo && isNbOrder(ordNo),
          included: false,
          reason: null,
          row: raw,
        };
    ...
    for (const r of rows) {
      if (r.isNb) {
        if (firstSeen.has(r.ordNo)) {
          const first = firstSeen.get(r.ordNo);
          r.included = false;
          r.reason =
            first.name === file.name
              ? 'Duplikat dalam file — kemunculan pertama yang dipakai'
              : `Duplikat — sudah diambil dari "${first.name}"`;
        } else {
          firstSeen.set(r.ordNo, { name: file.name, fileIdx: files.length });
          r.included = true;
        }
      } else {
        r.reason = 'Di luar pola NB — bukan U**N*';
      }
    }
    ...
    warnings.push(`"${file.name}" (${extracted.sheetName}): ${rows.filter((r) => r.included).length} baris NB diambil.`);
  }

  if (files.length === 0) {
    throw new Error(
      'Tidak ada file JIT yang bisa dibaca. Pastikan file berisi laporan produksi ' +
      'dengan kolom OrdNo & StyleNo. ' +
      (warnings.length ? `Detail: ${warnings.join(' | ')}` : '')
    );
  }
...
/** Rakit output hanya dari baris yang dipilih pengguna di panel tinjau.
 *  Urutan file = urutan unggah; header digabung hanya dari file yang
 *  masih punya baris terpilih; duplikasi dibiarkan sesuai pilihan user
 *  (dengan peringatan), lalu diurutkan U07→…→U06. */
export async function assembleFromSelection({ staged, stuffingFile = null, onProgress = () => {} }) {
  const warnings = [];
  const selectedFiles = staged.files
    .map((f) => ({ name: f.name, header: f.header, period: f.period, incRows: f.rows.filter((r) => r.included) }))
    .filter((f) => f.incRows.length > 0);

  if (selectedFiles.length === 0) {
    throw new Error('Tidak ada order yang dipilih — centang minimal satu order di panel tinjau.');
  }

  onProgress('Menggabungkan pilihan...');
  let mergedHeader = null;
  const rows = [];
  for (const { header, incRows } of selectedFiles) {
    if (!mergedHeader) mergedHeader = header.slice();
    let colMap = null;
    if (
      header.length !== mergedHeader.length ||
      header.some((h, i) => h !== mergedHeader[i])
    ) {
      colMap = header.map((h) => {
        const idx = mergedHeader.indexOf(h);
        if (idx >= 0) return idx;
    ...
    for (const srcRow of incRows.map((r) => r.row)) {
      const aligned = new Array(mergedHeader.length).fill(null);
      if (colMap) {
        srcRow.forEach((v, i) => { if (colMap[i] >= 0) aligned[colMap[i]] = v; });
      } else {
  ...
  // Duplikasi karena pilihan eksplisit user — transparan, tidak dihapus diam-diam
  const ordColMerged = mergedHeader.findIndex((h) => norm(h) === 'ordno');
  if (ordColMerged >= 0) {
    const dupCounts = new Map();
    for (const r of rows) {
      const o = String(r[ordColMerged] ?? '').trim();
      if (o) dupCounts.set(o, (dupCounts.get(o) || 0) + 1);
    }
    const dups = [...dupCounts.entries()].filter(([, n]) => n > 1);
    if (dups.length > 0) {
      const shown = dups.slice(0, 5).map(([o, n]) => `${o}×${n}`);
      warnings.push(`OrdNo sama diunduh lebih dari sekali sesuai pilihan tinjau (${dups.length} OrdNo ganda: ${shown.join(', ')}).`);
    }
  }

  const fallbackPeriod = `${new Date().getFullYear()} ${new Date().getMonth() + 1}`;
  let period = selectedFiles.map((f) => f.period).find(Boolean) || null;
  if (!period) {
    period = fallbackPeriod;
    warnings.push(`Periode tidak ditemukan di file terpilih — memakai bulan berjalan "${fallbackPeriod}".`);
  }
