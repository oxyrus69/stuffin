...
export async function processBlc({ jitFiles = [], stuffingFile = null, onProgress = () => {} }) {
  ...
  for (const file of jitFiles) {
    ...
    // Valid NB order: contains "NB" AND starts with "U" (PU… etc. discarded)
  ...
  if (!mergedHeader || rows.length === 0) {
    throw new Error('Tidak ada baris order NB yang bisa diekstrak dari file JIT yang diunggah.');
  }
  ...
  if (stuffingFile) {
    ...
    sheetName = `BLC HDU ${now.getMonth() + 1}.${String(now.getDate()).padStart(2, '0')} PAGI`.slice(0, 31);
    XLSX.utils.book_append_sheet(wbOut, XLSX.utils.aoa_to_sheet([[]]), sheetName);
    stagedBuf = XLSX.write(wbOut, { type: 'array', bookType: 'xlsx' });
  ...
  const data =
    mode === 'blc'
      ? styleSheetInZip(stagedBuf, { sheetName, ...styleMeta })
      : stagedBuf;
