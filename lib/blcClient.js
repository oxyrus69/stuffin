...
export async function processBlc({ jitFiles = [], stuffingFile = null, onProgress = () => {} }) {
  ...
  /* Sort: highest U-code first (U12 at top ... U01 at bottom).
     Stable sort keeps original JIT appearance order within each code group. */
  ...
  rows.map((row, i) => ({ row, i, code: codeOf(row) }))
